...
### Add image with the `img` tag

Now let's practice adding an image to a very simple app.  

Note that if you'd like to use a local image for with your app, you'll first have to save the image in a folder `www/` within your root directory. You'll see what we mean when you run the demo in the RStudio Cloud Project.
  ...
  tabsetPanel(
    ...
    tabPanel("Reference", 
             tags$p("There data were obtained from",
                    ...
                    tags$a("Rotten Tomatoes", href = "https://www.rottentomatoes.com/"), "."),
             tags$p("The data represent", nrow(movies), 
